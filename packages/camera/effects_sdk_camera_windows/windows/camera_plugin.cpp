...
constexpr char kSetBlurMethod[] = "setBlur";
constexpr char kClearBlurMethod[] = "clearBlur";
// constexpr char kSetSegmentationPresetMethod[] = "setSegmentationPreset";
constexpr char kSetBeautificationLevelMethod[] = "setBeautificationLevel";
constexpr char kClearBeautificationMethod[] = "clearBeautification";
constexpr char kSetBackgroundImageMethod[] = "setBackgroundImage";
...
constexpr char kBeautificationLevelKey[] = "beautificationLevel";
constexpr char kBackgroundImageKey[] = "backgroundImageUrl";
constexpr char kColorKey[] = "backgroundColor";
constexpr char kInitEffectsSDKKey[] = "effectsSDKPath";
...
std::optional<double> GetFloatValueOrNull(const EncodableMap& map,
                                         const char* key) {
  auto value = ValueOrNull(map, key);
  if (!value) {
    return std::nullopt;
  ...
  auto valfloat = std::get_if<double>(value);
  if (!valfloat) {
    return std::nullopt;
  }
  return *valfloat;
...
void CameraPlugin::HandleMethodCall(
  ...
  } else if (method_name.compare(kSetBlurMethod) == 0) {
    const auto* arguments =
        std::get_if<flutter::EncodableMap>(method_call.arguments());
    assert(arguments);

    return SetBlurMethodHandler(*arguments, std::move(result));
  } else if (method_name.compare(kClearBlurMethod) == 0) {
    const auto* arguments =
        std::get_if<flutter::EncodableMap>(method_call.arguments());
    assert(arguments);

    return ClearBlurMethodHandler(*arguments, std::move(result));
  } else if (method_name.compare(kSetBeautificationLevelMethod) == 0) {
    const auto* arguments =
        std::get_if<flutter::EncodableMap>(method_call.arguments());
    assert(arguments);

    return SetBeautificationLevelMethodHandler(*arguments, std::move(result));
  } else if (method_name.compare(kClearBeautificationMethod) == 0) {
    const auto* arguments =
        std::get_if<flutter::EncodableMap>(method_call.arguments());
    assert(arguments);

    return ClearBeautificationMethodHandler(*arguments, std::move(result));
  } else if (method_name.compare(kSetBackgroundImageMethod) == 0) {
    const auto* arguments =
        std::get_if<flutter::EncodableMap>(method_call.arguments());
    assert(arguments);

    return SetBackgroundImageMethodHandler(*arguments, std::move(result));
  } else if (method_name.compare(kSetBackgroundColorMethod) == 0) {
    const auto* arguments =
        std::get_if<flutter::EncodableMap>(method_call.arguments());
    assert(arguments);

    return SetBackgroundColorMethodHandler(*arguments, std::move(result));
  } else if (method_name.compare(kClearBackgroundMethod) == 0) {
    const auto* arguments =
        std::get_if<flutter::EncodableMap>(method_call.arguments());
    assert(arguments);
...
void CameraPlugin::SetBlurMethodHandler(const EncodableMap& args,
  ...
  if (camera->HasPendingResultByType(PendingResultType::kSetBlur)) {
    return result->Error("camera_error",
                         "Pending set blur request exists");
  }

  auto blur_power = GetFloatValueOrNull(args, kBlurPowerKey);
  if (!blur_power) {
    return result->Error("camera_error", "Can't parse blur power");
  }

  if (camera->AddPendingResult(PendingResultType::kSetBlur,
                               std::move(result))) {
    auto cc = camera->GetCaptureController();
    assert(cc);
    cc->SetBlur(static_cast<float>(blur_power.value()));
  }
}

void CameraPlugin::ClearBlurMethodHandler(const EncodableMap& args,
                                          std::unique_ptr<MethodResult<>> result) {
  auto camera_id = GetInt64ValueOrNull(args, kCameraIdKey);
  if (!camera_id) {
    return result->Error("argument_error",
  ...
  if (camera->HasPendingResultByType(PendingResultType::kClearBlur)) {
    return result->Error("camera_error",
                         "Pending clear blur request exists");
  }

  if (camera->AddPendingResult(PendingResultType::kClearBlur,
                               std::move(result))) {
    auto cc = camera->GetCaptureController();
    assert(cc);
    cc->ClearBlur();
...
void CameraPlugin::SetBeautificationLevelMethodHandler(const EncodableMap& args,
                                                       std::unique_ptr<MethodResult<>> result) {
  auto camera_id = GetInt64ValueOrNull(args, kCameraIdKey);
  if (!camera_id) {
    return result->Error("argument_error",
  ...
  if (camera->HasPendingResultByType(PendingResultType::kSetBeautification)) {
    return result->Error("camera_error",
                         "Pending set beautification request exists");
  }

  auto beautification_level = GetFloatValueOrNull(args, kBeautificationLevelKey);
  if (!beautification_level) {
    return result->Error("camera_error", "Can't parse beautification level");
  }

  if (camera->AddPendingResult(PendingResultType::kSetBeautification,
                               std::move(result))) {
    auto cc = camera->GetCaptureController();
    assert(cc);
    cc->SetBeautificationLevel(static_cast<float>(beautification_level.value()));
  }
}

void CameraPlugin::ClearBeautificationMethodHandler(const EncodableMap& args,
                                                    std::unique_ptr<MethodResult<>> result) {
  auto camera_id = GetInt64ValueOrNull(args, kCameraIdKey);
  if (!camera_id) {
    return result->Error("argument_error",
  ...
  if (camera->HasPendingResultByType(PendingResultType::kClearBeautification)) {
    return result->Error("camera_error",
                         "Pending clear beautification request exists");
  }

  if (camera->AddPendingResult(PendingResultType::kClearBeautification,
                               std::move(result))) {
    auto cc = camera->GetCaptureController();
    assert(cc);
    cc->ClearBeautification();
  }
}

void CameraPlugin::SetBackgroundImageMethodHandler(const EncodableMap& args,
                                                   std::unique_ptr<MethodResult<>> result) {
  auto camera_id = GetInt64ValueOrNull(args, kCameraIdKey);
  if (!camera_id) {
    return result->Error("argument_error",
  ...
  if (camera->HasPendingResultByType(PendingResultType::kSetBackgroundImage)) {
    return result->Error("camera_error",
                         "Pending set image request exists");
  }

  const auto* image_url =
      std::get_if<std::string>(ValueOrNull(args, kBackgroundImageKey));
  if (!image_url) {
    return result->Error("argument_error",
                         std::string(kBackgroundImageKey) + " argument missing");
  }

  if (camera->AddPendingResult(PendingResultType::kSetBackgroundImage,
                               std::move(result))) {
    auto cc = camera->GetCaptureController();
    assert(cc);
    cc->SetBackgroundImage(*image_url);
  }
}

void CameraPlugin::SetBackgroundColorMethodHandler(const EncodableMap& args,
                                                   std::unique_ptr<MethodResult<>> result) {
  auto camera_id = GetInt64ValueOrNull(args, kCameraIdKey);
  if (!camera_id) {
    return result->Error("argument_error",
  ...
  if (camera->HasPendingResultByType(PendingResultType::kSetBackgroundColor)) {
    return result->Error("camera_error",
                         "Pending set color request exists");
  }

  auto color = GetInt64ValueOrNull(args, kColorKey);
  if (!color) {
    return result->Error("camera_error", "Can't parse blur power");
  }

  if (camera->AddPendingResult(PendingResultType::kSetBackgroundColor,
                               std::move(result))) {
    auto cc = camera->GetCaptureController();
    assert(cc);
    cc->SetBackgroundColor(static_cast<int>(color.value())); 
  }
}

void CameraPlugin::ClearBackgroundMethodHandler(const EncodableMap& args,
                                                std::unique_ptr<MethodResult<>> result) {
  auto camera_id = GetInt64ValueOrNull(args, kCameraIdKey);
  if (!camera_id) {
    return result->Error("argument_error",
  ...
  if (camera->HasPendingResultByType(PendingResultType::kClearBackground)) {
    return result->Error("camera_error",
                         "Pending clear background request exists");
  }

  if (camera->AddPendingResult(PendingResultType::kClearBackground,
                               std::move(result))) {
    auto cc = camera->GetCaptureController();
    assert(cc);
    cc->ClearBackground();
...
void CameraPlugin::InitEffectsSDKMethodHandler(const EncodableMap& args,
                                               std::unique_ptr<MethodResult<>> result) {
  auto camera_id = GetInt64ValueOrNull(args, kCameraIdKey);
  if (!camera_id) {
    return result->Error("argument_error",
  ...
  const auto* lib_path =
      std::get_if<std::string>(ValueOrNull(args, kInitEffectsSDKKey));
  if (!lib_path) {
    return result->Error("argument_error",
                         std::string(kInitEffectsSDKKey) + " argument missing");
  }

  if (camera->AddPendingResult(PendingResultType::kInitEffectsSDK,
                               std::move(result))) {
    auto cc = camera->GetCaptureController();
    assert(cc);
