...
void CaptureControllerImpl::StartRecord(const std::string& file_path,
  ...
  // TODO: For capture modified by Effects SDK video
...
void CaptureControllerImpl::SetBlur(float blurPower) {
  if (!texture_handler_) {
    return;
  }
  texture_handler_->SetBlur(blurPower);

  capture_controller_listener_->OnSetBlur();
}

void CaptureControllerImpl::ClearBlur() {
  if (!texture_handler_) {
    return;
  }
  texture_handler_->ClearBlur();

  capture_controller_listener_->OnClearBlur();
}

void CaptureControllerImpl::SetBeautificationLevel(float level) {
  if (!texture_handler_) {
    return;
  }
  texture_handler_->SetBeautificationLevel(level);

  capture_controller_listener_->OnSetBeautification();
}

void CaptureControllerImpl::ClearBeautification() {
  if (!texture_handler_) {
    return;
  }
  texture_handler_->ClearBeautification();

  capture_controller_listener_->OnClearBeautification();
}

void CaptureControllerImpl::SetBackgroundImage(const std::string& url) {
  if (!texture_handler_) {
    return;
  }
  texture_handler_->SetBackgroundImage(url);

  capture_controller_listener_->OnSetBackgroundImage();
}

void CaptureControllerImpl::SetBackgroundColor(int color) {
  if (!texture_handler_) {
    return;
  }
  texture_handler_->SetBackgroundColor(color);

  capture_controller_listener_->OnSetBackgroundColor();
}

void CaptureControllerImpl::ClearBackground() {
  if (!texture_handler_) {
    return;
  }
  texture_handler_->ClearBackground();

  capture_controller_listener_->OnClearBackground();
}

void CaptureControllerImpl::InitEffectsSDK(const std::string& path) {
  if (!texture_handler_) {
    return;
  }
  texture_handler_->InitEffectsSDK(path);
