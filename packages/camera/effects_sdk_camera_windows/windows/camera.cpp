...
void CameraImpl::OnSetBlur() {
  auto pending_result =
      GetPendingResultByType(PendingResultType::kClearBlur);
  if (pending_result) {
    pending_result->Success();
  }
}

void CameraImpl::OnClearBlur() {
  auto pending_result =
      GetPendingResultByType(PendingResultType::kSetBlur);
  if (pending_result) {
    pending_result->Success();
  }
}

void CameraImpl::OnSetBeautification() {
  auto pending_result =
      GetPendingResultByType(PendingResultType::kClearBeautification);
  if (pending_result) {
    pending_result->Success();
  }
}

void CameraImpl::OnClearBeautification() {
  auto pending_result =
      GetPendingResultByType(PendingResultType::kSetBeautification);
  if (pending_result) {
    pending_result->Success();
  }
}

void CameraImpl::OnSetBackgroundImage() {
  auto pending_result =
      GetPendingResultByType(PendingResultType::kClearBackground);
  if (pending_result) {
    pending_result->Success();
  }
}

void CameraImpl::OnSetBackgroundColor() {
  auto pending_result =
      GetPendingResultByType(PendingResultType::kClearBackground);
  if (pending_result) {
    pending_result->Success();
...
void CameraImpl::OnClearBackground() {
  auto pending_result =
      GetPendingResultByType(PendingResultType::kSetBackgroundImage);
  if (pending_result) {
    pending_result->Success();
  }

  pending_result = GetPendingResultByType(PendingResultType::kSetBackgroundColor);
  if (pending_result) {
    pending_result->Success();
  }
}

void CameraImpl::OnInitEffectsSDK() {
  auto pending_result =
      GetPendingResultByType(PendingResultType::kInitEffectsSDK);
  if (pending_result) {
    pending_result->Success();
  }
}
