...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> _fetchCameras() async {
    ...
    int cameraIndex = 0;
    try {
      cameras = await CameraEffectsSDKPlatform.instance.availableCameras();
      if (cameras.isEmpty) {
  ...
  Future<void> _initializeCamera() async {
    ...
    try {
      ...
      cameraId = await CameraEffectsSDKPlatform.instance.createCamera(
        camera,
        _resolutionPreset,
      ...
      _errorStreamSubscription?.cancel();
      _errorStreamSubscription = CameraEffectsSDKPlatform.instance.onCameraError(cameraId).listen(_onCameraError);

      _cameraClosingStreamSubscription?.cancel();
      _cameraClosingStreamSubscription = CameraEffectsSDKPlatform.instance.onCameraClosing(cameraId).listen(_onCameraClosing);

      final Future<CameraInitializedEvent> initialized = CameraEffectsSDKPlatform.instance.onCameraInitialized(cameraId).first;

      await CameraEffectsSDKPlatform.instance.initializeCamera(
        cameraId,
      );
      ...
      try {
        if (cameraId >= 0) {
          await CameraEffectsSDKPlatform.instance.dispose(cameraId);
        }
      } on CameraException catch (e) {
  ...
  Future<void> _disposeCurrentCamera() async {
    if (_cameraId >= 0 && _initialized) {
      try {
        await CameraEffectsSDKPlatform.instance.dispose(_cameraId);
  ...
  Widget _buildPreview() {
    return CameraEffectsSDKPlatform.instance.buildPreview(_cameraId);
  }

  Future<void> _takePicture() async {
    final XFile file = await CameraEffectsSDKPlatform.instance.takePicture(_cameraId);
  ...
  Future<void> _recordTimed(int seconds) async {
    if (_initialized && _cameraId > 0 && !_recordingTimed) {
      CameraEffectsSDKPlatform.instance.onVideoRecordedEvent(_cameraId).first.then((VideoRecordedEvent event) async {
        if (mounted) {
          setState(() {
            _recordingTimed = false;
          });
      ...
      await CameraEffectsSDKPlatform.instance.startVideoRecording(
        _cameraId,
  ...
  Future<void> _toggleRecord() async {
    if (_initialized && _cameraId > 0) {
      if (_recordingTimed) {
        ...
        await CameraEffectsSDKPlatform.instance.stopVideoRecording(_cameraId);
      } else {
        if (!_recording) {
          await CameraEffectsSDKPlatform.instance.startVideoRecording(_cameraId);
        } else {
          final XFile file = await CameraEffectsSDKPlatform.instance.stopVideoRecording(_cameraId);
  ...
  Future<void> _togglePreview() async {
    if (_initialized && _cameraId >= 0) {
      if (!_previewPaused) {
        await CameraEffectsSDKPlatform.instance.pausePreview(_cameraId);
      } else {
        await CameraEffectsSDKPlatform.instance.resumePreview(_cameraId);
      }
